...
TextToSpeech.prototype.speakPhrase = function(phrase, speechLanguage){
  ...
  // works without setting .voice to meet the logic written above e.g. speechLanguage (es-BO) for Bolivia speak phrase works even though there is no voice for (es-BO) so speechSynthesis must be using a fall back of another 'es' speaking voice instead. For Serbia (sr-RS), there is no voice for this language but the word is spoken with an English voice.
  // I assume for more complicated languages like Bengali (bn-BD) there is no voice as it's not possible for the English voice to pronounce the text ?
